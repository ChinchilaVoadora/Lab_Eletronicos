...
int main(int argc, char *argv[]) {
    ...
    while ((waitKey(1)&0xff)!=27) { // ESC=27 sai do programa
        ...
        while (scale <= width/3 && scale <= height/3) {
            resize(T_cc, res_T_cc, Size(scale, scale), 1, 1, INTER_NEAREST);
            resize(T_ncc, res_T_ncc, Size(scale, scale), 1, 1, INTER_NEAREST);
