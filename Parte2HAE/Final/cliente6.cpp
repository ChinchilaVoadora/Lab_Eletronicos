...
void drawInfo(Mat_<COR>& info, float fps, float m, int label, int indice, MNIST &mnist) {
    ...
    if (label == -1 || label > 10) 
        putText(info, "-", Point(95, 25), 1, 1, branco);
    else
    ...
    Mat_<FLT> match(14, 14);
    if (indice == -1) 
        match.setTo(Scalar(1));
    else    
        match = mnist.ax.row(indice).reshape(1, mnist_size);
...
const float turnTime = 1.4;
void turnLeft(int count) {
    estadoTurning = 4;
    turning = true;
    stopTime = timeSinceEpoch() + turnTime*count;
}
...
int main(int argc, char *argv[]) {
    ...
    if (argv[3][0] == 'c')
        ...
        vo = VideoWriter(argv[2], CV_FOURCC('X','V','I','D'), 15, Size(2*width,height*7/6));
    ...
    while ((waitKey(1)&0xff)!=27) { // ESC=27 sai do programa
        ...
        Mat_<COR> number_cor;
        Mat_<FLT> number;
        if (center.x != 0 && center.y != 0 && scale > width*0*3) {
            ...
            prevLabel = -1;
            indice = -1;
            croppedNumber.setTo(Scalar(1));
        }

        int number_state = -1;
        if (value < 14 && value != 0 && label == prevLabel) {
            number_state = label;
        }
        else {
        ...
        m = estado;
        if (estado == 2 && center.x < width/2 && center.x != 0)
            m = 1;
        else if (estado == 2 && center.x > width/2 && center.x != 0)
            m = 3;
        ...
        //Mat_<FLT> teste(14, 28);
        //hconcat(croppedNumber, mnist.ax.row(indice).reshape(1,14), teste);
