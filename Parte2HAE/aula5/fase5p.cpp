...
int main(int argc, char *argv[]) {
    ...
    Mat_<int> matches(mnist.nq,1); Mat_<float> dists(mnist.nq,1);
    ...
    while ((waitKey(1)&0xff)!=27) { // ESC=27 sai do programa
        ...
        std::cout << "hjk\n\n";
        //Mat_<COR> number_cor = camera(Rect((center.x - scale*0.2/2), (center.y - scale*0.2/2), scale*0.3, scale*0.3));
        Mat_<COR> number_cor = imread("nove.png");
        Mat_<FLT> number;
        ...
        int coord_cols = 1000;
        int coord_rows = 1000;
        for (int i = 0; i < number.cols; ++i) {
            for (int j = 0; j < number.rows; ++j) {
                if (number[j][i] == 0) {
                    if (i < coord_cols)
                        coord_cols = i;
        ...
        if (height > width) {
            ...
            coord_cols = coord_cols - (height - width) / 2;
            if (coord_cols < 0) coord_cols = 0;
            number = number(Rect(coord_cols, coord_rows, new_width, height));
        }
        else {
            int new_height = width;
            coord_rows = coord_rows - (width - height) / 2;
            if (coord_rows < 0) coord_rows = 0;
            number = number(Rect(coord_cols, coord_rows, width, new_height));    
        }


        resize(number, number, Size(14, 14), INTER_NEAREST);
        
        
        Mat flat = number.reshape(1, 1);
        std::vector<int> indices(1);      // nearest neighbor index
        std::vector<float> dists2(1);      // distance

        ind.knnSearch(flat,indices,dists2,1,flann::SearchParams(32));

        int predictedLabel = mnist.ay(indices[0]);
        
        Mat teste;
        hconcat(number, mnist.ax.row(indices[0]).reshape(1, 14), teste);
        imshow("janela", teste);
        waitKey(0);
        std::cout << "Predicted digit: " << predictedLabel << std::endl;
        uint32_t m = estado;
        //client.sendUint(m);
