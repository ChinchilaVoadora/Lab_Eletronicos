...
int main(int argc, char *argv[]) {
    ...
    while ((waitKey(1)&0xff)!=27) { // ESC=27 sai do programa
        ...
        int turning_treshold = 45;
        int estado_follow = 0;
        if (center.x < width/2 - turning_treshold) {
            estado_follow = 1;
            std::cout << "ESQUERDA\n";
            std::cout << center.x << " menor que " <<  width/2 - turning_treshold << endl;

        }
        else if (center.x > width/2 + turning_treshold) {
            estado_follow = 3;
            std::cout << "DIREITA\n";
            std::cout << center.x << " maior que " <<  width/2 + turning_treshold << endl;
